...
public abstract class FilterTemplate implements Filterer{
  ...
  public void filterBatch(int valueCount){
    doTransfers();
    switch(svMode){
    case NONE:
      filterBatchNoSV(valueCount);
      break;
    case TWO_BYTE:
      filterBatchSV2(valueCount);
      break;
    default:
      throw new UnsupportedOperationException();
    }
  ...
  private void filterBatchSV2(int valueCount){
    int svIndex = 0;
    final int count = valueCount*2;
  ...
  private void filterBatchNoSV(int valueCount){
    int svIndex = 0;
    for(char i =0; i < valueCount; i++){
