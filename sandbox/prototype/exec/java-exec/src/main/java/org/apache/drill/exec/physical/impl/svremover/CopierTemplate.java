...
public abstract class CopierTemplate implements Copier{
  ...
  private void allocateVectors(int valueCount){
  ...
  public void copyRecords(){
    final int valueCount = sv2.getCount();
    allocateVectors(valueCount);
    int outgoingPosition = 0;
    
    for(int svIndex = 0; svIndex < valueCount * 2; svIndex++, outgoingPosition++){
