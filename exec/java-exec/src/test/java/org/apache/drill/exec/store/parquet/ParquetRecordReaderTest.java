...
public class ParquetRecordReaderTest extends BaseTestQuery {
  ...
  @Test
  public void testNoColumnSelect() throws Exception {
    List<QueryResultBatch> results = testSqlWithResults("select 1+2 from dfs.`" + fileName + "`");
    ...
    IntVector vector = (IntVector) loader.getValueAccessorById(IntVector.class, 0).getValueVector();

    assert vector.getAccessor().getValueCount() == 300;
