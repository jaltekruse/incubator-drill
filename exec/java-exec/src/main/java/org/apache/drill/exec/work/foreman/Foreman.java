...
public class Foreman implements Runnable, Closeable, Comparable<Object>{
  ...
  private void parseAndRunPhysicalPlan(String json) {
    try {
      ...
      setOptions(plan.getProperties().drillOptions);
  ...
  private void setOptions(JSONOptions drillOptionMap) {
    if (drillOptionMap != null) {
      JsonNode globalOptions = drillOptionMap.getRoot().get("global");
      ...
      JsonNode sessionOptions = drillOptionMap.getRoot().get("session");
