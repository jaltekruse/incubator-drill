...
public class JsonReader {
  ...
  public JsonReader(List<SchemaPath> columns, boolean allTextMode) throws JsonParseException, IOException {
  ...
  private void writeData(MapWriter map) throws JsonParseException, IOException {
    ...
    outside: while(true){
      ...
      case VALUE_NULL:
        if (allTextMode) {
          handleString("null", map, fieldName);
          break;
        }
  ...
  private void handleString(String value, MapWriter writer, String fieldName) throws IOException {
    VarCharHolder vh = new VarCharHolder();
    writer.varChar(fieldName).write(prepareVarCharHolder(vh, parser.getText()));
  }
